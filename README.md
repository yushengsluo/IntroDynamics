...
Open `dist/index.html` directly in Safari or another modern browser. Keep the entire `dist` folder together so navigation, styles, images, and scripts remain available. Both apps include packaged scripts for local `file://` URLs; no browser security changes are needed.
...
3. Add a **Back to examples** link in the application with `href="../../index.html"`.
...
See the [Linear flows documentation](docs/linear-flows.md) and [Forced oscillator documentation](docs/forced-oscillator.md) for features, source modules, and numerical limits.
